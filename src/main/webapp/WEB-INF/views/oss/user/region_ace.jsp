		...
		<div id="role-table_wrapper" class="dataTables_wrapper" role="grid">
			<table id="role-table"
				...
				<tr role="row">
					...
					<th class="sorting"><small style="font-size: small;">地区名</small></th>
					<th class="sorting"><small style="font-size: small;">地区标示</small></th>
					<th class="sorting"><small style="font-size: small;">url</small></th>
					<th class="sorting"><small style="font-size: small;">css-key</small></th>
				...
				<c:forEach var="r" items="${regionList}">
					<tr>
						...
						<td>${r.districtValue}</td>
						<td>${r.indexUrl}</td>
						<td>${r.cssKey}</td>
						<td><a
								href="region/regionChangeDialog?id=${r.id}&activeType=2"
								data-toggle="modal" class="btn btn-xs btn-warning"
								data-target="#roleChangeDialog">修改地区信息</a>
						</td>
					</tr>
				</c:forEach>
				</tbody>
...
<!-- /.main-container -->
<!-- 模态框（Modal） -->
<form id="userChangeForm" method="post" action="region/regionChange"
	...
	jQuery(function($) {
		var oTable1 = $('#role-table').dataTable({
			...
			"fnInitComplete": function() {
				$("#mytool").append('<a href="region/regionChangeDialog?activeType=1" data-toggle="modal" class="btn btn-sm btn-info" data-target="#roleChangeDialog">添加地域</a>');
			}
		});
